...
public class WeightMeasurementTest {
    ...
    void testConversion_RoundTrip() {
    	...
    	WeightMeasurement roundTrip = original.convertTo(WeightUnit.GRAM).convertTo(WeightUnit.KILOGRAM);
        assertEquals(original.getValue(), roundTrip.getValue(), EPSILON);
    }

    @Test
    void testAddition_SameUnit() {
    	WeightMeasurement result = new WeightMeasurement(1.0, WeightUnit.KILOGRAM).add(new WeightMeasurement(2.0, WeightUnit.KILOGRAM));
        assertEquals(3.0, result.getValue(), EPSILON);
    }

    @Test
    void testAddition_CrossUnit() {
    	WeightMeasurement result = new WeightMeasurement(1.0, WeightUnit.KILOGRAM).add(new WeightMeasurement(1000.0, WeightUnit.GRAM));
        assertEquals(2.0, result.getValue(), EPSILON);
    }

    @Test
    void testAddition_ExplicitTarget() {
    	WeightMeasurement result =WeightMeasurement.add(new WeightMeasurement(1.0, WeightUnit.KILOGRAM), new WeightMeasurement(1000.0, WeightUnit.GRAM), WeightUnit.GRAM);
    ...
    void testAddition_Negative() {
    	WeightMeasurement result = new WeightMeasurement(5.0, WeightUnit.KILOGRAM).add(new WeightMeasurement(-2000.0, WeightUnit.GRAM));
        assertEquals(3.0, result.getValue(), EPSILON);
    }

    @Test
    void testAddition_Zero() {
    	WeightMeasurement result = new WeightMeasurement(5.0, WeightUnit.KILOGRAM).add(new WeightMeasurement(0.0, WeightUnit.GRAM));
        assertEquals(5.0, result.getValue(), EPSILON);
    }

    @Test
    void testNullUnit() {
        assertThrows(IllegalArgumentException.class,() -> new WeightMeasurement(1.0, null));
    }

    @Test
    void testInvalidValue() {
        assertThrows(IllegalArgumentException.class,() -> new WeightMeasurement(Double.NaN, WeightUnit.KILOGRAM));
    }
}